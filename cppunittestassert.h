...
namespace Microsoft{ namespace VisualStudio {namespace CppUnitTestFramework
{
   ...
   template<class T, typename std::enable_if<hasToString<T>::value, int>::type = 0> static std::wstring ToString(const T& t)
   {
      std::string asString = t.toString();
      return std::wstring(asString.begin(), asString.end()).c_str();
   }

   template<class T, typename std::enable_if<std::is_pod<T>::value, int>::type = 0> static std::wstring ToString(const T& t)
   {
   ...
   template<class T, typename std::enable_if<std::is_same<std::string, typename std::remove_cv<T>::type>::value, int>::type = 0> static std::wstring ToString(const T& t)
   {
      return std::wstring(t.begin(), t.end()).c_str();
   }
